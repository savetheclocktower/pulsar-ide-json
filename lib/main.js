...
class JSONLanguageClient extends AutoLanguageClient {
  ...
  async sendSchemaAssociations () {
    if (!this._server) return;
    let associations = [...SCHEMAS];
    for (let value of Object.values(this.customAssociations)) {
      associations.push(value);
    }
    this.logger.log('Sending json/schemaAssociations:', associations);
    try {
      await this._server.connection.sendCustomNotification(
        'json/schemaAssociations',
        associations
      );
    } catch (err) {
      // This can throw if we try to send after the connection is disposed.
      // Often we find out in enough time that the server has disconnected, but
      // sometimes the timing is unlucky.
      console.warn('Failed to set schema associations:', err);
    }
  }
  ...
  async getHover (editor, point) {
    let result = await super.getHover(editor, point);
    if (!result) return result;

    // FIX: The JSON language service does an insane thing: it encodes all
    // spaces as `&nbsp;`. This is wrong for several reasons, so let’s just
    // switch it back.
    let contents = result.contents;
    if (contents) {
      let { kind, value } = contents;
      contents = {
        kind,
